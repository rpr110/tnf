...
class FaceproofLogs(Base,SerializerMixin):
    __table__ = metadata.tables["Faceproof_Logs"]
    company = relationship("Company")
    status = relationship("StatusMaster")
    service = relationship("ServiceMaster")
    __mapper_args__ = { 'primary_key': [metadata.tables["Faceproof_Logs"].c.public_id] }
